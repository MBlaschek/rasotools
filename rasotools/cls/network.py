...
class Network(object):
    ...
    def __repr__(self):
        head = "%s  (%d)" % (self.name, len(self.idents))
        body = ""
    ...
    def get_data_files(self, name, pattern=None, directory=None, **kwargs):
        ...
        status = pd.DataFrame({'status': False, 'file': ''}, index=self.stations.index)
        for ifile in files[:]:
            inside = False
            for ident in self.idents:
                if ident in ifile:
                    inside = True
                    status.loc[ident, 'status'] = True
                    status.loc[ident, 'file'] = ifile
                    break
            if not inside:
                files.remove(ifile)
        self.files[name] = status
        message("Summary: ", self.files[name].status.sum(), '/', len(self.idents), **kwargs)
    ...
    def load_data(self, name, files=None, pattern=None, directory=None, variables=None, invert_selection=False, npp=10,
        ...
        from ..fun import find_files, levelup, message
        from .. import config
        ...
        if len(files) > 0:
            ...
            if combine:
                ...
                self.data[name].attrs['title'] = 'RNetwork({}, {}) - {}'.format(self.name, len(self.idents),
                                                                                files[0].replace('.nc', ''))
    ...
    def apply_function(self, variables=None, npp=10, **kwargs):
        ...
        # kwargs  : exec
        stuff = make_process_list(self.idents, process_function, args=(variables,), **levelup(**kwargs))
    ...
    def filter_stations(self, logic, apply=False):
        logic = np.asarray(logic)
        new_stations = self.stations.iloc[logic]
        if len(new_stations) > 0:
            if apply:
                print("Subsetting ...")
                self.idents = list(np.asarray(self.idents)[logic])
                self.lon = self.lon[logic]
                self.lat = self.lat[logic]
                self.stations = self.stations.iloc[logic]
            else:
                print("Possible subset:")
                print(self.stations.iloc[logic])
                print(self.stations.iloc[~logic])
    ...
    def availability(self, filename=None, reference_year=None):
        if hasattr(self, 'stations'):
            if sum(self.stations.columns.isin(['start', 'end'])) == 2:
                imin = self.stations['start'].min()
                imax = self.stations['end'].max()
                summary = "Availability of %d Stations between %d - %d \n" % (len(self.idents), imin, imax)
